...
class ColorFunctions():
    ...
    def interpolate_gradient(self, mute_color, max_volume_color, max_volume):
        c1 = self.hex2rgb(mute_color)
        c2 = self.hex2rgb(max_volume_color)
        return [self.rgb2hex((c1 + t*(c2-c1)).astype(int))
                for t in np.linspace(0, 1, max_volume)]
...
class Py3status:
    ...
    def post_config_hook(self):
        ColFuncions = ColorFunctions()
        self.gradient_dict = ColFuncions.interpolate_gradient(
                self.mute_color, self.max_volume_color, self.max_volume)
    ...
    # uses generated dict to choose color according to volume percentage
    def _perc_to_color(self, string):
        vol = int(string)
        if 0 <= vol <= self.max_volume - 1:
            return self.gradient_dict[vol]
        else:
            return self.max_volume_color
